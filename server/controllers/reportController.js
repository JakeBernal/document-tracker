  ...
  try {
    const summaryRows = await query(`
      SELECT
        ...
        SUM(CASE
          WHEN status IN ('Approved', 'Ready for Pickup')
          THEN 1 ELSE 0
    ...
    return res.json({
      summary: {
        total_requests: Number(summaryRows[0]?.total_requests || 0),
        pending_requests: Number(summaryRows[0]?.pending_requests || 0),
        approved_requests: Number(summaryRows[0]?.approved_requests || 0),
        completed_requests: Number(summaryRows[0]?.completed_requests || 0),
        total_payment_collected: Number(
          summaryRows[0]?.total_payment_collected || 0
        ),
        total_system_fee_revenue: Number(
          summaryRows[0]?.total_system_fee_revenue || 0
        ),
      },
